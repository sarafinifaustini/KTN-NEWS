...
class _BusinessPageState extends State<BusinessPage>  with AutomaticKeepAliveClientMixin {
  final controller = ScrollController();
  ...
  Future<List<Videos>> getVideos() async {
    try {
      ...
      if (response.statusCode == 200) {
        List jsonResponse = json.decode(response.body)['videos'];
        mainVideos = jsonResponse;
        BusinessPage.liveThumb = mainVideos[0]['thumbnail'];
        BusinessPage.playingTitle = mainVideos[0]['title'];
        // NewsPage.playingVideo = mainVideos[0]['id'];
        print(BusinessPage.playingVideo);
        print("hapo juu");
        print(BusinessPage.liveThumb);
        print(jsonResponse);
        return jsonResponse.map((data) => new Videos.fromJson(data)).toList();
      } else {
        throw Exception('Unexpected error occurred !');
  ...
  onScroll() {
    if (controller.position.atEdge) {
      if (controller.position.pixels == 0) {
        ...
        if(this.mounted) {
          setState(() {
            BusinessPage.cHeight = BusinessPage.screenHeight * 0.35;
          });
        }
      }
      //else if at bottom
      ...
      if(this.mounted){
        setState(() {
          BusinessPage.cHeight = BusinessPage.screenHeight * 0.2;
        });
      }}
  }
  ...
  static Future refreshBusiness(context) async {
    // print("in refresh");
    return APICalls.getVideos("/ktn-news/videos/22/0/20");
  }

  @override
  Widget build(BuildContext context) {
    BusinessPage.screenHeight = MediaQuery.of(context).size.height;
    Size size = MediaQuery.of(context).size;

    BusinessPage.cHeight = BusinessPage.screenHeight*0.3;
    // NewsPage.playingVideo = mainVideos[0]['id'];
    return RefreshIndicator(
        onRefresh: () => APICalls.refreshLiveStream(context),
        child: YoutubeVideo(
