...
class _SportsPageState extends State<SportsPage>  with AutomaticKeepAliveClientMixin {
  ...
  Future<List<Videos>> getVideos() async {
    try {
      ...
      if (response.statusCode == 200) {
        List jsonResponse = json.decode(response.body)['videos'];
        mainVideos = jsonResponse;
        SportsPage.liveThumb = mainVideos[0]['thumbnail'];
        SportsPage.playingTitle = mainVideos[0]['title'];
        // NewsPage.playingVideo = mainVideos[0]['id'];
        print(SportsPage.playingVideo);
        print("hapo juu");
        print(SportsPage.liveThumb);
        print(jsonResponse);
        return jsonResponse.map((data) => new Videos.fromJson(data)).toList();
      } else {
        throw Exception('Unexpected error occurred !');
  ...
  onScroll() {
    if (controller.position.atEdge) {
      if (controller.position.pixels == 0) {
        ...
        if(this.mounted) {
          setState(() {
            SportsPage.cHeight = SportsPage.screenHeight * 0.35;
          });
        }
      }
      //else if at bottom
      ...
      if(this.mounted){
        setState(() {
          SportsPage.cHeight = SportsPage.screenHeight * 0.2;
        });
      }}
  }
  ...
  static Future refreshSportLiveStream(context) async {
    // print("in refresh");
    return APICalls.getVideos("/ktn-news/videos/23/0/20");
  }

  @override
  Widget build(BuildContext context) {
    SportsPage.screenHeight = MediaQuery.of(context).size.height;
    Size size = MediaQuery.of(context).size;

    SportsPage.cHeight = SportsPage.screenHeight*0.3;
    // NewsPage.playingVideo = mainVideos[0]['id'];
    return RefreshIndicator(
        onRefresh: () =>refreshSportLiveStream(context),
        child: YoutubeVideo(
          child: Expanded(
            child: Container(
                // color: Theme.of(context).scaffoldBackgroundColor,
